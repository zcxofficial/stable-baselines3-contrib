...
class MaskableTRPO(OnPolicyAlgorithm):
    ...
    def collect_rollouts(
        ...
        # 检查 rollout_buffer 是否是 MaskableRolloutBuffer 或 MaskableDictRolloutBuffer 类型的实例。如果不是，会抛出一个错误，错误消息为 “RolloutBuffer doesn’t support action masking”。这个错误提示表明 rollout_buffer 不支持动作屏蔽（action masking）功能。
        assert isinstance(
    ...
    def train(self) -> None:
        ...
        for rollout_data in self.rollout_buffer.get():
            ...
            tmp_policy = copy.deepcopy(self.policy)

            values, log_prob, entropy = tmp_policy.evaluate_actions(
            ...
            while F.kl_div(log_prob,th.exp(rollout_data.old_log_prob)).cpu().detach().numpy() > 1.1e-06:
                entropy_losses = []
                pg_losses, value_losses = [], []
                tmp_policy = copy.deepcopy(self.policy)
                # Normalize advantage
                ...
                # Optimization step
                tmp_policy.optimizer.zero_grad()
                loss.backward()
                # Clip grad norm
                th.nn.utils.clip_grad_norm_(tmp_policy.parameters(), self.max_grad_norm)
                tmp_policy.optimizer.step()
        ...
        learning_rate_backup = self.learning_rate
        ...
        self.learning_rate = learning_rate_backup
